...
TELEMT_COMMON_PRIV="${TELEMT_REPO_ROOT}/common/telemt_privilege.sh"
...
if [[ -f "${TELEMT_COMMON_PRIV}" ]]; then
  source "${TELEMT_COMMON_PRIV}"
else
  echo "telemt: telemt_privilege.sh not found: ${TELEMT_COMMON_PRIV}" >&2
  exit 1
fi
